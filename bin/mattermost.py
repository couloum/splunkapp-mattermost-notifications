...
def send_notification(msg, url, attachment=None):
    ...
    if attachment:
        ...
        print >> sys.stderr, 'DEBUG Adding attachment to body with attachment=%s' % (url, body)
        body = json.dumps(data)
...
def table_broker(payload):
    print >> sys.stderr, "INFO Sending message with payload %s" % payload
    settings = payload.get('configuration')
    print >> sys.stderr, "INFO Sending message with settings %s" % settings
    ...
    if table:
        ...
        if table == "table":
            ...
            saved_search_name = settings.get('search_name')
            ...
            trigger_date = settings.get('trigger_date')
            print >> sys.stderr, "DEBUG Trigger date: %s" % trigger_date
            trigger_epoch = settings.get('trigger_epoch')
            print >> sys.stderr, "DEBUG Trigger epoch: %s" % trigger_epoch
            trigger_epoch_value = datetime.fromtimestamp(trigger_epoch)
